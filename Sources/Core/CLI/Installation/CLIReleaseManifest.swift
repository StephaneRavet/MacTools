...
enum CLIInstallError: String, Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupported: "仅支持 Apple 芯片 Mac 上的已签名 Nightly 版本。"
        case .metadata: "此版本缺少有效的 CLI 安装信息，请更新 Nightly 后重试。"
        ...
        case .ownership: "无法确认 CLI 安装的归属，已停止更改。请保留文件并检查安装路径。"
        case .collision: "mactools-nightly 路径已被占用。请自行移动现有命令后重试。"
        case .filesystem: "无法安全完成 CLI 文件操作，请检查可用空间和目录权限后重试。"
        case .busy: "另一项 CLI 安装操作正在进行，请稍后重试。"
        case .validation: "CLI 连接验证失败，已尝试恢复上一版本。请检查命令行集成和后台运行权限后重试。"
        }
    }
}
