...
- `Nightly`：启用后每天 UTC 06:00 从 `main` 构建。它复用稳定发布已有的签名、公证、Sparkle 和插件 catalog Secrets，生成可与稳定版共存的 `MacTools Nightly.app`、单独下载的 Apple silicon `mactools` CLI、同一提交的完整插件集、独立 appcast 和独立 PluginKit v5 catalog。CLI 在无 Secrets、无仓库写入凭据的独立 job 中执行验证；发布 job 只下载同一份不可变 workflow artifact。每次运行使用唯一 `nightly-<run>-<attempt>` tag；先以 draft 上传并核对全部产物，再发布为 prerelease，绝不覆盖已有 tag 或资产。
...
3. 打开 `Actions` → `Nightly`，以 `main` 手动运行一次。检查签名、公证、GitHub prerelease、`docs/nightly/appcast.xml`、`docs/nightly/plugins/v5/catalog.json` 和 Pages 部署均成功；从同一个 prerelease 下载 DMG、CLI ZIP 及两个 checksum，按 `docs/testing/cli-nightly-distribution.md` 完成签名、Gatekeeper、架构和版本验证；安装该 DMG，并从 Nightly catalog 安装至少一个插件。
